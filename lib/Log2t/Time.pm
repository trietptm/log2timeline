...
#                       Time
#########################################################################################
# This is a small library to assist with time manipulation
#
...
# small subroutine that returns the nanoseconds of a Windows FILETIME
...
# Taken from the dos2unixtime function from the tsk3/fs/fatfs_meta.c file from
# The Sleuthkit.
# The logic and code taken there, and adapted to be a Perl code (the other is a
# C code)
sub Dos2Unix($$)
{
		...
		if( ($sec < 0 ) || ($sec > 60 ) )
		{
			print STDERR "[TIME DOS2UNIX] Seconds not correctly formed ($sec).  Resetting to zero value\n";
			$sec = 0;
		}
		...
		if( ($min < 0 ) || ($min > 59 ) )
		{
			print STDERR "[TIME DOS2UNIX] Minutes not correctly formed ($min).  Resetting to zero value\n";
			$min = 0;
		}
		...
		if( ($hour < 0 ) || ($hour > 23 ) )
		{
			print STDERR "[TIME DOS2UNIX] Hour not correctly formed ($hour).  Resetting to zero value\n";
			$hour = 0;
		}
		...
		if( ($month < 1 ) || ($month > 12 ) )
		{
			print STDERR "[TIME DOS2UNIX] Month not correctly formed ($month).  Resetting to zero value\n";
			$month = 1;
		...
		if( ($year < 0 ) || ($year > 137 ) )
		{
			print STDERR "[TIME DOS2UNIX] Year not correctly formed ($year).  Resetting to zero value\n";
			$year = 0;
		}
		else	
		{
		...
		{
			print STDERR "[TIME DOS2UNIX] Timestamp is WITHOUT ANY INFORMATION REGARDING TIME, ONLY THE DATE IS ACCURATE\n";
		}

		#printf STDERR "[TIME DOS2UNIX] DATE 0x%x TIME 0x%x\n", $date, $time;
		#print STDERR "[TIME DOS2UNIX] y $year m $month d $day - h $hour m $min s $sec\n";
...
sub iso2epoch($$)
{
	...
	#print STDERR "[iso2epoch] ISO TO EXAMINE [$iso]\n";

        # modify the date into Epoch, its in ISO 8601 format
        # 2009-08-08T19:07Z
        ($a,$b) = split( /T/,$iso );

	#print STDERR "[iso2epoch] PART I [$a] PART II [$b]\n";
	...
	#print STDERR "[iso2epoch] AFTER CONVERTING: YEAR $year MONTH $month DAY $day HOUR $hour + $add_h MINUTE $min + $add_m  SECOND $sec TIMEZONE $tz\n";
...
sub epoch2cftl($$)
{
	...
	return sprintf "%0.4d-%0.2d-%0.2d %0.2d:%0.2d:%0.2d.0", $iso->year,$iso->month,$iso->day,$iso->hour, $iso->minute, $iso->second;
}
...
sub epoch2text($$$)
{
	...
	return ( (sprintf "%0.2d/%0.2d/%0.4d", $iso->month,$iso->day,$iso->year), (sprintf "%0.2d:%0.2d:%0.2d", $iso->hour, $iso->minute, $iso->second)) if $use_local == 3;
...
#	return $date->epoch;
	...
	if( $stamp =~ /(\d{4})-([a-zA-Z]+)-(\d{1,2})\s(\d{1,2}):(\d{1,2}):(\d{1,2})\s([a-zA-Z\+\-0-9]+)/ )
	{
		# make some checking
	...
	elsif( $stamp =~ /(\d{4})-([a-zA-Z]+)-(\d{1,2})\s(\d{1,2}):(\d{1,2}):(\d{1,2})\.?\d+?\s([a-zA-Z\+\-0-9]+)/ )
	{
		# make some checking
		return 0 unless ( $3 < 32 && $3 > 0 );
		return 0 unless ( $4 < 25 && $4 > -1 );
...
sub epoch2iso($$)
{
	...
	return sprintf "%0.4d-%0.2d-%0.2dT%0.2d:%0.2d%0.2dZ",$iso->year, $iso->month, $iso->day, $iso->hour, $iso->minute,$iso->second;
}
...
sub text2epoch($$) {
	...
	my $tz = shift; # added by Kristinn
        my $analyzer = DateTime::Format::Strptime->new( pattern => '%a %b %d %H:%M:%S %Y', time_zone => $tz ); # time zone information added by Kristinn
...
sub csv2epoch($$) {
	...
	my $tz = shift; # added by Kristinn
        ...
        my $analyzer = DateTime::Format::Strptime->new( pattern => '%m/%d/%Y %H:%M:%S', time_zone => $tz ); # time zone information added by Kristinn
        my $dt = $analyzer->parse_datetime($string);
        if ($dt) {
                return $dt->epoch;
        } else {
...
sub fix_epoch($$)
{
	...
	$val = 8 if $dt->is_dst();
        ...
        eval
        {
                ...
                $a = DateTime->new( year => 2000, month => $val, day => 1, hour => 12, minute => 0, second => 0, time_zone => $tz );

                # then we construct another datetime object, now using UTC as our baseline
                $b = DateTime->new( year => 2000, month => $val, day => 1, hour => 12, minute => 0, second => 0, time_zone => 'UTC' );
...
{
	# get the time
	my ($second, $minute, $hour, $dayOfMonth, $m, $year, $dayOfWeek, $dayOfYear, $daylightSavings) = localtime();
	...
	# and return it
	return sprintf "%0.2d:%0.2d:%0.2d, %s %s %0.2d %0.4d ", $hour,$minute,$second,$weekDays[$dayOfWeek],$months[$m],$dayOfMonth,$year;
}
...
=head1 NAME
 ...
 Log2t::Time - A library that assists with time manipulation (part of log2timeline)

=head1 METHODS

=over 4

=item Win2Unix( low, high )

This function, written by Andreas Schuster and Csaba Barta, takes as an argument Windows filetime date and returns a date in the Epoch format

=item iso2epoch( string )

Accepts a string that contains a date formatted according to the ISO 8601 and returns an epoch time
