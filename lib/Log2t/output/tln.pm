...
sub get_description()
{
	return "Output timeline using H. Carvey's TLN format"; 
}
...
sub print_line()
{
        ...
        if( scalar( %{$t_line} ) )
	{
		...
		$notes .= ' inode:' . $t_line->{'extra'}->{'inode'} if defined $t_line->{'extra'}->{'inode'};

		# change \ to /
		$notes =~ s/\\/\//g;

		# create a prefix field
		$prefix = $t_line->{'sourcetype'} eq $t_line->{'source'} ? '' : '(' . $t_line->{'sourcetype'} . ') ';
		...
		foreach( keys %{$t_line->{'time'}} )
		{
			...
			::print_line( $t_line->{'time'}->{$_}->{'value'} . "|" . $t_line->{'source'} . '|' . $t_line->{'extra'}->{'host'} . "|" . $t_line->{'extra'}->{'user'} . '|' . $p2 .  $t_line->{'desc'} . '|' . $self->{'short_tz'} . '|' . $notes . "\n" ) if $t_line->{'time'}->{$_}->{'value'} > 0;
		}
	}
	else
	{
