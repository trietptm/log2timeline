...
sub _process_line
{
	...
	$text = $t_line->{'source'} eq 'FILE' ? '[' . $self->{'mactime'} . '] ' :  '[' . $t_line->{'sourcetype'} . '] ';
	$text .= '(' . $self->{'type'} . ') ' unless $self->{'type'} eq '';
	$text .= '<' . $t_line->{'extra'}->{'host'} . '> ' if ( defined $t_line->{'extra'}->{'host'} and $t_line->{'extra'}->{'host'} ne 'unknown' );
	$text .= 'User: ' . $t_line->{'extra'}->{'user'} . ' ' if ( defined $t_line->{'extra'}->{'user'} && $t_line->{'extra'}->{'user'} ne 'unknown' );
	$text .= $t_line->{'desc'};
	...
	$text =~ s/,/-/g; # exchange the commas for -, to make the change of mactime output to csv easier (mactime doesn't like , inside the output)
	$text =~ s/;/_/g; # exchange the ; for _, to make the change of mactime output to csv easier (Excel doesn't like ; inside the output)
	...
	::print_line( $md5. "|". $text. "|". $inode. "|". $mode. "|". $uid. "|". $gid. "|". $size. "|". $self->{'atime'}. "|". $self->{'mtime'}. "|". $self->{'ctime'}. "|". $self->{'crtime'}. "\n" );
}
		...
		if( $t_line->{'source'} eq 'FILE' )
		{
			...
			# go through each defined time to find the mactime (and type)
				...
				if( $t_line->{'time'}->{$_}->{'type'} =~ m/SI/ )
				{
					...
					$self->{'mtime'} = $t_line->{'time'}->{$_}->{'value'} if $t_line->{'time'}->{$_}->{'legacy'} & 0b0001;
					$self->{'atime'} = $t_line->{'time'}->{$_}->{'value'} if $t_line->{'time'}->{$_}->{'legacy'} & 0b0010;
					$self->{'ctime'} = $t_line->{'time'}->{$_}->{'value'} if $t_line->{'time'}->{$_}->{'legacy'} & 0b0100;
					$self->{'crtime'} = $t_line->{'time'}->{$_}->{'value'} if $t_line->{'time'}->{$_}->{'legacy'} & 0b1000;
				}
				else
				{
					# filename
					$mtime = $t_line->{'time'}->{$_}->{'value'} if $t_line->{'time'}->{$_}->{'legacy'} & 0b0001;
					$atime = $t_line->{'time'}->{$_}->{'value'} if $t_line->{'time'}->{$_}->{'legacy'} & 0b0010;
					$ctime = $t_line->{'time'}->{$_}->{'value'} if $t_line->{'time'}->{$_}->{'legacy'} & 0b0100;
					$crtime = $t_line->{'time'}->{$_}->{'value'} if $t_line->{'time'}->{$_}->{'legacy'} & 0b1000;
				}
			}
