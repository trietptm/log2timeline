...
sub print_line()
{
        ...
        if( scalar( %{$t_line} ) )
	{
		...
		$notes = 'File:' . $t_line->{'extra'}->{'path'} . $t_line->{'extra'}->{'filename'} if ( defined $t_line->{'extra'}->{'filename'} && -f $t_line->{'extra'}->{'filename'} );
		$notes = 'Dir:' . $t_line->{'extra'}->{'path'} . $t_line->{'extra'}->{'filename'} if ( defined $t_line->{'extra'}->{'filename'} && -d $t_line->{'extra'}->{'filename'} );
