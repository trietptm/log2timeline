...
sub print_header()
{
	...
	{
		print STDERR "Unable to create database, please provide a file name to write database to (use log2timeline with the -w FILE parameter)\n";
		return 0;
	}

	# connect to a database
	$self->{'db'} = DBI->connect("dbi:SQLite:dbname=$db_file", "", "", { RaiseError => 1, AutoCommit => 1 }) or $return = 0;
...
$self->{'db'}->do( $sql ) or $return = 0;
	...
	$sql = "INSERT INTO info (version,tool,ver_tool) VALUES( '$VERSION' , 'log2timeline', '" . Log2t::Common::get_version . '\')';
	$self->{'db'}->do( $sql ) or $return = 0;
...
{
	# get the host ID
	$self->_get_hid( $t_line->{'extra'}->{'host'} ) if $t_line->{'extra'}->{'host'} ne $self->{'host'}->{'name'};

	# get the source ID
	$self->_get_sid( $t_line->{'source'}, $t_line->{'sourcetype'} ) if $t_line->{'sourcetype'} ne $self->{'source'}->{'detail'};

	# get the filename ID
	$self->_get_fid( $t_line->{'extra'}->{'filename'} ) if $t_line->{'extra'}->{'filename'} ne $self->{'file'}->{'name'};
	...
	foreach( keys %{$t_line->{'time'}} )
	{
		...
		$sql = q{INSERT INTO records (short,detailed,srcid,legacy,description,inode,time,user,fid,sid,hid,hidden) VALUES( ?,?,?,?,?,?,?,?,?,?,?,0)};
