        ...
        if( scalar( %{$t_line} ) )
	{
		#printf STDERR "[PRINT] M %d A %d C %d B %d\n",$mtime,$atime,$ctime,$btime;
			...
			next unless $t_line->{'time'}->{$_}->{'value'} > 0;
                	...
                	$text .= '(' . $t_line->{'time'}->{$_}->{'type'} . ' - ' . $mactime . ') ' unless $convert;
                	$text .= 'User: ' . $t_line->{'extra'}->{'user'} . ' ' if ( defined $t_line->{'extra'}->{'user'} && $t_line->{'extra'}->{'user'} ne 'unknown' );
			$text .= 'Host: ' . $t_line->{'extra'}->{'host'} . ' ' if ( defined $t_line->{'extra'}->{'host'} && $t_line->{'extra'}->{'host'} ne 'unknown' );
                	$text .= $t_line->{'desc'};
                	$text .= ' (file: ' . $t_line->{'extra'}->{'path'} . $t_line->{'extra'}->{'filename'} . ')';

			# remove all possible tabs in the text
			$text =~ s/\t/ /g;

			# add the timestamp to the output
                	$text .= "\t" . Log2t::Time::epoch2text( $t_line->{'time'}->{$_}->{'value'}, 2, $self->{'tz'} ) . "\t" . Log2t::Time::epoch2text( $t_line->{'time'}->{$_}->{'value'}, 2, $self->{'tz'} );
