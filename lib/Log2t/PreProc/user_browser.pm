...
sub get_info($)
{
	...
	foreach my $root ( @dir )
	{
		...
		if( ( $croot =~ m/^users$/i or $croot =~ m/^documentsandsettings$/i ) and -d $l2t->{'file'} . $l2t->{'sep'} . "$root" )
		{
			...
			foreach my $user ( @userdir )
			{
				if ( -d $l2t->{'file'} . $l2t->{'sep'} . $root . $l2t->{'sep'} .  $user )
				{
					...
					{
						if ( -f $l2t->{'file'} . $l2t->{'sep'} . $root . $l2t->{'sep'} . $user . $l2t->{'sep'} . $nt and $nt =~ m/ntuser.dat/i )
						{
							$go = 1;
							$user_files{lc( $user )} = $l2t->{'file'} . $l2t->{'sep'} . $root . $l2t->{'sep'} . $user . $l2t->{'sep'} . $nt;
						}
					}
				}
			}
	...
	foreach my $user ( keys %user_files )
	{
		...
		eval 
		{
				...
				$l2t->{'defbrowser'}->{$user} = $key->get_value("")->get_data() unless $key->get_value("")->get_data() eq '';
				print STDERR "[PreProcessing] The default browser of user $user according to registry is: (" . $l2t->{'defbrowser'}->{$user} . ")\n";
        		}   
