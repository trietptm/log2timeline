...
sub get_info($)
{
	...
	foreach my $root ( @dir )
	{
		if( ( $root =~ m/^windows$/i or $root =~ m/^winnt$/i ) and -d $l2t->{'file'} . $l2t->{'sep'} . $root )
		{
			$path = $root . $l2t->{'sep'};
			...
			{
				if ( -d $l2t->{'file'} . $l2t->{'sep'} . $path . $l2t->{'sep'} . $win  and $win =~ m/^system32$/i )
				{
					$path .= $l2t->{'sep'} . $win;
					...
					{
						if ( -d $l2t->{'file'} . $l2t->{'sep'} . $path . $l2t->{'sep'} . $sys and $sys =~ m/^config$/i )
						{
							$path .= $l2t->{'sep'} . $sys;
							$go = 1;
						}
	...
	if( -f $reg_file2 )
	{
		...
		eval 
		{
			...
			print STDERR "[PreProcessing] The default system browser is: : " . $l2t->{'defbrowser'}->{'os'} . "\n";
		};
		if( $@ )
		{
