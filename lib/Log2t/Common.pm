...
sub list_input()
{
	...
	opendir( PD, $folder . '/Log2t/input' ) || die( "Could not open the directory '$folder/Log2t/input/'\n" );
	...
	foreach (@dir)
	{
		       ...
		       $name =~ s/\.pm//g;;
                       ...
                       $ret .= sprintf "%20s\t%4s\t%s\n", $name, $module->get_version(), $module->get_description();
               };
...
sub list_output()
{
	...
	# open up the input module folder to find all the available input modules
	opendir( PD, $folder . '/Log2t/output' ) || die( "Could not open the directory '$folder/Log2t/output/'\n" );
	@dir = readdir( PD );
	@dir = sort( @dir );
	closedir( PD );	
