...
sub get_time
{
	...
	foreach my $key ( keys %{$self->{'files'}} )
	{
		...
		$self->{'rp_file'} = ${$self->{'name'}} . $self->{'sep'} . $self->{'files'}->{$key} . $self->{'sep'} . 'rp.log';
		eval
		{
			$rpinfo = $self->_read_rpfile;
		};
		if( $@ )
		{
			print STDERR "[RP] Problems reading the restore point " . $self->{'files'}->{$key} . ". Skipping it...\n";
			next;
		}
		...
		next unless (defined $rpinfo);
        	...
        	$self->{'container'}->{$self->{'cont_index'}++} = {
        	        'time' => { 0 => { 'value' => $rpinfo->{'date'}, 'type' => 'Created', 'legacy' => 15 } },
        	        'desc' => 'Restore point ' . $self->{'files'}->{$key} . ' created - ' . $rpinfo->{'name'},
...
sub verify
{
	...
	eval
	{
		...
		%file_hash  = map { $self->{'count'}++ => $_ } grep { /^RP/ && -d ${$self->{'name'}} . $self->{'sep'} . $_ } readdir( $self->{'file'} );
	...
	foreach ( keys %file_hash )
	{
		...
		if( $file_hash{$_} =~ m/^RP/ )
		{
			$found = 1 if -f ${$self->{'name'}} . $self->{'sep'} . $file_hash{$_} . $self->{'sep'} . 'rp.log';
			$rp = $_;
		}
	}
	...
	if( $found )
	{
		...
		{
			$return{'success'} = 0;
			$return{'msg'} .= 'No rp.log file inside restore point, perhaps not a restore point directory?. ';
		}
	}
	else
	{
