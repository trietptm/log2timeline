...
# GPL license (same license as log2timeline), so his code is now dual-licensed.
#
...
sub get_time() {
    ...
    my %si_time;
    my %fn_time;
    my ($read_ptr, $atr_record, $si_record, $fn_record, $volume_info_record, $object_id_record,
        $al_record);
    my $mftr;
    my ($si_text, $fn_text);
    my %info = undef;
    ...
    printf STDERR "[MFT] Error, bad MFT Header (0x%x)\n", $mftr->{'magic'}
      if (($mftr->{'magic'} == 0x44414142) && ($self->{'debug'}));
    return \%t_line if $mftr->{'magic'} == 0x44414142;

    printf STDERR "[MFT] Error, bad MFT Header (0x%x)\n", $mftr->{'magic'}
      if (($mftr->{'magic'} != 0x454C4946) && ($self->{'debug'}));
    ...
    while ($read_ptr < $self->{'record_length'}) {
        ...
        if ($atr_record->{'type'} == 0x10)    # Standard Information
        {
            printf STDERR
              ...
              $atr_record->{'nlen'}, $atr_record->{'name_off'}
              if $self->{'debug'};

            $si_record =
              $self->_decodeSIAttribute(substr $self->{'record'},
                                        $read_ptr + $atr_record->{'soff'});
            $mftr->{'si'} = $si_record;
            printf STDERR "++CRTime: %d\n++MTime: %d\n++ATime: %d\n++EntryTime: %d\n",
              $si_record->{'crtime'}, $si_record->{'mtime'}, $si_record->{'atime'},
              $si_record->{'ctime'}
              if $self->{'debug'};
        }
        elsif ($atr_record->{'type'} == 0x20)    # Attribute list
        {
            ...
            if ($atr_record->{'res'} == 0) {
                $al_record =
                  $self->_decodeAttributeList(substr $self->{'record'},
                                              $read_ptr + $atr_record->{'soff'});
                $mftr->{'al'} = $al_record;
        ...
        elsif ($atr_record->{'type'} == 0x30)    # File name
        {
            ...
            $fn_record =
              $self->_decodeFNAttribute(substr $self->{'record'},
                                        $read_ptr + $atr_record->{'soff'});
            $mftr->{'fn'}    = $fn_record;
            ...
            if ($fn_record->{'crtime'} != 0) {
                printf STDERR "\tCRTime: %s MTime: %s ATime: %s EntryTime: %s",
                  $fn_record->{'crtime'}, $fn_record->{'mtime'}, $fn_record->{'atime'},
                  $fn_record->{'ctime'}
                  if $self->{'debug'};
            }
        }
        ...
        {
            $object_id_record =
              $self->_decodeObjectID(substr $self->{'record'}, $read_ptr + $atr_record->{'soff'});
        ...
        elsif ($atr_record->{'type'} == 0x70)    # Volume information
        {
            ...
            $volume_info_record =
              $self->_decodeVolumeInfo(substr $self->{'record'}, $read_ptr + $atr_record->{'soff'});
...
sub _decodeSIAttribute {
    ...
    # read the variable
    $d{'crtime'} =
      Log2t::Time::Win2Unix(unpack("V", substr $s, 0, 4), unpack("V", substr $s, 4, 4));
    $d{'mtime'} =
      Log2t::Time::Win2Unix(unpack("V", substr $s, 8, 4), unpack("V", substr $s, 12, 4));
    $d{'ctime'} =
      Log2t::Time::Win2Unix(unpack("V", substr $s, 16, 4), unpack("V", substr $s, 20, 4));
    $d{'atime'} =
      Log2t::Time::Win2Unix(unpack("V", substr $s, 24, 4), unpack("V", substr $s, 28, 4));
    ...
    $d{'crtime_empty'} =
      Log2t::Time::getNanoWinFileTime(unpack("V", substr $s, 0, 4), unpack("V", substr $s, 4, 4)) ==
      0 ? 1 : 0;
    $d{'mtime_empty'} =
      Log2t::Time::getNanoWinFileTime(unpack("V", substr $s, 8, 4), unpack("V", substr $s, 12, 4))
      == 0 ? 1 : 0;
    $d{'ctime_empty'} =
      Log2t::Time::getNanoWinFileTime(unpack("V", substr $s, 16, 4), unpack("V", substr $s, 20, 4))
      == 0 ? 1 : 0;
    $d{'atime_empty'} =
      Log2t::Time::getNanoWinFileTime(unpack("V", substr $s, 24, 4), unpack("V", substr $s, 28, 4))
      == 0 ? 1 : 0;
...
sub _decodeFNAttribute {
    ...
    $d{'crtime'} =
      Log2t::Time::Win2Unix(unpack("L", substr $s, 8, 4), unpack("L", substr $s, 12, 4));
    $d{'mtime'} =
      Log2t::Time::Win2Unix(unpack("L", substr $s, 16, 4), unpack("L", substr $s, 20, 4));
    $d{'ctime'} =
      Log2t::Time::Win2Unix(unpack("L", substr $s, 24, 4), unpack("L", substr $s, 28, 4));
    $d{'atime'} =
      Log2t::Time::Win2Unix(unpack("L", substr $s, 32, 4), unpack("L", substr $s, 36, 4));
