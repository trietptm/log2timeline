...
#  16 => extra
    ...
    %t_line = (
               'time' => { 0 => { 'value' => $epoch, 'type' => $bits[6], 'legacy' => $legacy }, },
               'desc'       => $bits[10],
               'short'      => $bits[9],
               'source'     => $bits[4],
               'sourcetype' => $bits[5],
        ...
        if ($#words eq 16) {
            ...
            if ($words[0] =~ m/^\d{2}\/\d{2}\/\d{4}$/) {
                ...
                if ($words[11] == 2) {
                    ...
                    $return{'success'} = 0;
                    $return{'msg'} =
                      'The timestamp object version was wrong (should be equal to 2 not '
                      . $words[11] . ')';
                }
                ...
                $return{'success'} = 0;
                $return{'msg'} =
                  'Not the correct date format (should be MM/DD/YYYY not [' . $words[0] . '])';
            }
        }
        else {
            $return{'success'} = 0;
            $return{'msg'}     = "The file is not of the correct format (" . sprintf "%d",
              $#words + 1 . " fields instead of 17)";
        }
