...
sub get_time
{
  ...
  $status = $self->{'zip'}->extractMember( '_rels/.rels', $self->{'temp'} . $self->{'sep'} . 'rels.xml' );
  print STDERR "Unable to extract schema from file [" . ${$self->{'name'}} . "], is it really a Office 2007 document (openXML)?\n" if $status != AZ_OK;
  ...
  foreach $attrib (@targets)
  {
    ...
    if( $@ )
    {
      print STDERR "[OXML] Unable to properly parse one of the OpenXML metadata XML files.  The file $attrib was the cause of all this trouble.
Please review this file manually.  The error message given is:\n$@\n";
  ...
  foreach( keys %{$self->{'lines'}} )
  {
    ...
    if( $self->{'lines'}->{$_} =~ m/\d{4}-\d{2}-\d{2}/ )
    {
      ...
      $self->_process_timestamp or print STDERR "[OXML] Error while processing timestamp for $_ (" . $self->{'lines'}->{$_} . "\n";
    }
  }
...
sub _process_timestamp
{
  ...
  $text .= ' (' . $self->{'lines'}->{'title'} . ')' if ( defined $self->{'lines'}->{'title'} ) and ( $self->{'lines'}->{'title'} ne '' );
  $text .= ' - ' . $self->{'lines'}->{'subject'} . ' - ' if ( defined $self->{'lines'}->{'subject'} ) and ( $self->{'lines'}->{'subject'} ne '' );
  $text .= ' - ' . $self->{'lines'}->{'description'} . ' - ' if ( defined $self->{'lines'}->{'description'} ) and ( $self->{'lines'}->{'description'} ne '' );
  $text .= ' - Application: ' . $self->{'lines'}->{'application'} if defined $self->{'lines'}->{'application'};
  $text .= ' - Company: ' . $self->{'lines'}->{'company'} if defined $self->{'lines'}->{'company'} and $self->{'lines'}->{'company'} ne '';
  $text .= ' - AppVersion: ' . $self->{'lines'}->{'appversion'} if defined $self->{'lines'}->{'appversion'} and $self->{'lines'}->{'appversion'} ne '';
...
sub verify
{
  ...
  eval
  {
    ...
    if( $self->{'info'}->{'magic'} eq 0x04034b50 )
    {
      ...
      $self->{'info'}->{'filename'} = Log2t::BinRead::read_ascii($self->{'file'},\$ofs,$self->{'info'}->{'filename_length'} );
...
sub _process_file
{
  ...
  $status = $self->{'zip'}->extractMember( $xmlfile, $self->{'temp'} . $self->{'sep'} . 'file.xml' ) ;
  if ( $status != AZ_OK )
  {
    print STDERR "[OXML ERROR] Unable to extract MetaData from file, is it really a Office 2007 document?\n";
    return 0;
  }
  ...
  foreach $property (@properties)
  {
    ...
    if( $property->nodeType==ELEMENT_NODE )
    {
      ...
      if( $#splits eq 1 )
      {
        ...
        $self->{'lines'}->{lc( $splits[1] )} = encode( $self->{'encoding'}, $property->textContent);
      }
      else
      {
        print STDERR "[OXML] Pushing key " . lc( $splits[0] ) . " into lines\n" if $self->{'debug'};
        $self->{'lines'}->{lc( $splits[0]) } = encode( $self->{'encoding'}, $property->textContent);;
      }
