...
#		SECURITY	
#################################################################################################
# This script parses the SECURITY registry file
#
...
sub _parse_default
{
	...
	$self->{'container'}->{$self->{'cont_index'}++} = {
			...
			0 => { 'value' => $ts, 'type' => 'Last Written', 'legacy' => 15 },
                ...
                'desc' => "Key name: HKLM/SECURITY$name" ,
                'short' => $name,
                'source' => 'REG',
                'sourcetype' => 'SECURITY key',
                'version' => 2,
                'extra' => {  }
        };
...
sub new()
{
	...
	$self->{'verify_key'} = 'Policy\\PolAdtEv';
...
sub get_description()
{
	return "Parses the SECURITY registry file";	
}
...
sub get_help()
{
	return "This input module parses the SECURITY registry file.
