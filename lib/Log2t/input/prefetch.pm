...
sub get_time
{
	...
	{
		if(  ($self->{'files'}->{$key} =~ m/^\.$/ ) or ($self->{'files'}->{$key} =~ m/^\.\.$/ ) or ($self->{'files'}->{$key} =~ m/Layout.ini/i ) )
		{
		...
		else
		{
			...
			$fpath = undef;
			($runcount,$runtime,$exe,$fpath) = $self->_getMetaData( ${$self->{'name'}} . $self->{'sep'} . $self->{'files'}->{$key} );
