...
sub get_time()
{
	...
	if($self->{'type'}==1)
	{
		...
		$text = 'Filename: '.$content[3].(($content[0] ne '')?' | Manufacturer: '.$content[0]:'').(($content[1] ne '')?' | Internal Name: '.$content[1]:'').(($content[2] ne '')?' | File Version: '.$content[2]:'').(($content[4] ne '')?' | Product Name: '.$content[4]:'').(($content[5] ne '')?' | Known As: '.$content[5]:'');
		$text .= (($content[7] ne '')?' | Domain: '.$content[7]:'').' | Discovered: '.(($discDT)?($discDT->ymd.' '.$discDT->hms):('Unavailable')).' | Last Start: '.(($lsStartDT)?($lsStartDT->ymd.' '.$lsStartDT->hms):('Unavailable'));
		if($content[11] != 0)
		{
			$text .= ' | Run Count: '.$content[11].'  | Total Run Time: '.$content[12].' | Avg CPU Usage: '.$content[13].' | Peak Memory: '.$content[14];
			#print "|$11 | $12 | $13 | $14\n"
		}
	...
	elsif($self->{'type'}==3)
	{
		...
		# build a useful description string
		$text = 'PID: '.$content[0].' | File Path: '.$content[2].' | Domain: '.$content[4].' | Total Execution Time: '.$content[6].' | Peak Memory Usage: '.$content[7].' | Avg CPU Usage: '.$content[8];
		$user = $content[3];
		#$host = $content[4];
