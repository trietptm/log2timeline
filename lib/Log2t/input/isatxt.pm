...
sub get_time
{
  ...
  if( $#fields ne $#words )
  {
    ...
    print STDERR "Supposed fields are $#fields yet the line only contains $#words - there might be some disrepencies in the output line\n";
...
sub verify
{
  ...
  while( $tag )
  {
    $tag = 0 unless $format_line = Log2t::BinRead::read_ascii_until( $self->{'file'}, \$ofs, "\n", 600 );
