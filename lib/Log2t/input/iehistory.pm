...
sub _parse_timestamp {
  ...
  # Last Accessed Time Stamp - 0x10 0x10 8 This is the Last Accessed time stamp, in FILETIME format
        ...
        $ofs += 3;
        printf STDERR "[IE] Directory reading [0x%x]: 0b%b ", $ofs - 4, $r{'dir_in'}
          if $self->{'debug'} > 1;
...
#$r{'header'} = Log2t::BinRead::read_ascii_end($self->{'file'},\$ofs,$r{'length'}-$r{'header_ofs'});
        $r{'header'} =
          Log2t::BinRead::read_ascii_magic($self->{'file'}, \$ofs, $r{'length'} - $r{'header_ofs'},
                                           "\r\n\r\n");
...
#$text .= 'URL:' . $r{'url'} . ' cache stored in: ' . $r{'dir'} . '/' . $r{'filename'} . ' - ' . $r{'header'} .'';
    ...
    if ($r{'time1'} == $r{'time2'}) {
        ...
        # same time
        $t_line{'time'} =
          { 0 => { 'value' => $r{'time1'}, 'type' => $time1 . '/' . $time2, 'legacy' => 15 }, };
    }
    else {

        # time is different
        $t_line{'time'} = {
                            0 => { 'value' => $r{'time1'}, 'type' => $time1, 'legacy' => 14 },
                            1 => { 'value' => $r{'time2'}, 'type' => $time2, 'legacy' => 1 }
                          };
    }
    ...
    if (defined $self->{'defbrowser'}->{ lc($r{'user'}) }) {
        $t_line{'notes'} =
          $self->{'defbrowser'}->{ $r{'user'} } =~ m/iexplore/
          ? 'Default browser for user'
          : 'Not the default browser (' . $self->{'defbrowser'}->{ $r{'user'} } . ')';
    }
    elsif ($self->{'defbrowser'}->{'os'} ne '') {

        # check the default one (the OS)
        $t_line{'notes'} =
          $self->{'defbrowser'}->{'os'} =~ m/iexplore/
...
sub get_help() {
    return
      "This plugin parses the index.dat file that contains Internet activities as recorder by Internet Explorer.";
...
sub _read_header {
    ...
    $self->{'struct'}->{'version_string'} =
      Log2t::BinRead::read_ascii_end($self->{'file'}, \$ofs, 40);
