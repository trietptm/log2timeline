...
my @daysbefore = ( 0, 31, 59, 90, 120, 151, 181, 212, 243, 273,
		   304, 334 );
my @monthlength = ( 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30,
		    31 );
...
sub analog_time($) {
	...
	for ($month = $DEC; 
	     $daysbefore[$month] + 
	        (_is_leapyear($year) && $month > $FEB) > $time;
	     $month--) {
	}
	
	$day = $time - $daysbefore[$month] + 
	    1 - (_is_leapyear($year) && $month > $FEB);
    }
...
sub get_time
{
	...
	my $pattern_header = "^CACHE type 5 produced by analog 6.0/Unix. Do not modify or delete!\\s*\$";
	my $pattern_req_summary = "^T\\s+(\\d+)\\s+(\\d+)\\s+(\\d+)\\s+(\\d+)\\s+(\\d+)\\s+(\\d+)\\s+(\\d+)\\s*\$"; # 7 columns
	my $pattern_date_summary = "^D\\s+(\\d+)\\s+(\\d+)\\s*\$"; # 2 columns
	my $pattern_activity = "^(\\d+)\\s+(\\d+)\\s+(\\d+)\\s+(\\d+)\\s*(\\d+)\\s*\$"; # 4 columns
	my $pattern_item = "^(\\d+)\\s+(\\d+)\\s+(\\d+)\\s+(\\d+)\\s+(\\d+)\\s+(\\d+)\\s+(\\d+)\\s+(\\d+)\\s+(\\d+)\\s+(\\d+)\\s+(\\d+)\\s+(\\d+)\\s+(.*)\$"; # 12 columns
	my $pattern_z = "^z\\s+(\\d+)\\s+(\\d+)\\s+(\\d+)\\s+(\\d+)\\s+(\\d+)\\s*\$"; # 5 columns
	my $pattern_codes = "^c\\s+(\\d+)\\s+(\\d+)\\s+(\\d+)\\s*\$"; # 3 columns
	my $pattern_p = "^p\\s+(\\d+)\\s+(\\d+)\\s+(\\d+)\\s+(\\d+)\\s+(\\d+)\\s+(\\d+)\\s*\$"; # 6 columns
	...
	while(<$fh>) {
		...
		$ret_lines{$ret_index++} = {
		    'time' => { 0 => { 'value' => $first_time, 'type' => 'Entry written', 'legacy' => 15 } },
		...
		$ret_lines{$ret_index++} = {
		    'time' => { 0 => { 'value' => $last_time, 'type' => 'Entry written', 'legacy' => 15 } },
