...
# 127.0.0.1 - - [24/Jun/2010:22:02:28 -0400] "GET /pagead/test_domain.js HTTP/1.1" 404 336 "http://docstore.mik.ua/orelly/perl/cookbook/ch20_13.htm" "Mozilla/5.0 (X11; U; Linux i686; en-US; rv:1.9.0.19) Gecko/2010040118 Ubuntu/8.10 (intrepid) Firefox/3.0.19"
	...
	if ($line =~ /^([0-9\.]+) ([^\s]+) ([^\s]+) \[(\d+)\/(\w\w\w)\/(\d+):(\d+):(\d+):(\d+) ([\+\-]\d+)\] "([^"]+)" ([\d-]+) ([\d-]+) "([^"]+)" "([^"]+)".*/ )
	{
		$li{'c-ip'} = $1;
		$li{'cs-username'} = $2;
		$li{'userid'} = $3;		
		...
		print STDERR "[URLSNARF] IP:", $li{'c-ip'}, ' USERNAME:', $li{'cs-username'}, ' USERID:', $li{'userid'}, ' DAY:', $li{'day'}, ' M:[', $li{'month'}, '] Y:', $li{'year'}, ' H:', $li{'hour'}, ' M:', $li{'min'}, ' S:', $li{'sec'}, ' TZ:', $li{'tz'}, ' Request:', $li{'cs-request'}, ' Status:', $li{'sc-status'}, ' Bytes:', $li{'sc-bytes'}, ' REF:', $li{'cs(Referer)'}, ' UA:', $li{'cs(User-Agent)'}, "\n" if $self->{'debug'};
        ...
        %t_line = (
                ...
                'version' => 2.4,
                'extra' => { 'user' => $li{'userid'}, 'host' => $li{'c-ip'}, 'src-ip' => $li{'c-ip'}, 'dst-ip' => $li{'s-ip'}, 'size' => $li{'cs-bytes'} }
        );
