...
my %translate = (
  ...
  '9' => 'URL what was generated from a replacable keyword other than the default search provider',
...
sub new()
{
  ...
  $self->{'chrome_date_ofs'} = 11644473600000000;# the offset of Chrome's date function to the std. Epoch (stored in WEBKIT format)
...
sub _parse_timestamp
{
  ...
  if ( $self->{'r_type'} eq 'download' )
  {
    ...
    $text .= $r_line[1] . ' (' . $r_line[0] . ').  Total bytes received : ' . $r_line[3] . ' (total: ' . $r_line[4] . ')';
    $date = $r_line[2];
    $size = $r_line[4];
  ...
  elsif ( $self->{'r_type'} eq 'url' )
  {
    ...
    if( $from->{'s_url'} ne '' )
    {
      $text .= $r_line[0] . ' (' . $r_line[1] . ') [count: ' . $r_line[2] . '] Host: ' . $hostname . ' visited from: ' . $from->{'s_url'};
    }
    else
    {
      $text .= $r_line[0] . ' (' . $r_line[1] . ') [count: ' . $r_line[2] . '] Host: ' . $hostname;
    }
  
    # and include the visit transition 
    $text .= ' type: [' . $transition{ $r_line[8] & $self->{'CORE_MASK'} } . ' - ' . $translate{ $r_line[8] & $self->{'CORE_MASK'} } .  ']' ;
  ...
  {
    $t_line{'notes'} = $self->{'defbrowser'}->{$self->{'username'}} =~ m/chrome/i ? 'Default browser for user' : 'Not the default browser (' . $self->{'defbrowser'}->{$self->{'username'}} . ')';
  }
        elsif ( $self->{'defbrowser'}->{'os'} ne '' )
        {   
                # check the default one (the OS)
                $t_line{'notes'} = $self->{'defbrowser'}->{'os'} =~ m/chrome/ ? 'Default browser for system' : 'Not the default system browser (' . $self->{'defbrowser'}->{'os'} . ')';
        } 
...
WHERE
  ...
  $return{'s_date'} = Log2t::Numbers::roundup( ($return{'s_date'} - $self->{'chrome_date_ofs'} ) / 1000000 );
