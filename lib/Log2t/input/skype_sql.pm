...
sub get_time
{
  ...
  $sql = "SELECT Chats.friendlyname,Messages.author,Messages.from_dispname,Messages.body_xml,Messages.timestamp,Messages.dialog_partner 
FROM Chats,Messages
...
# Tengja vid Messages tofluna
  ...
  $sql = "SELECT timezone_policy,fullname,country,given_displayname,lastonline_timestamp,skypename FROM Accounts";
  $sth = $self->{'vdb'}->prepare( $sql );
  $result = $sth->execute( );
...
sub _parse_timestamp
{
  ...
  if ( $self->{'r_type'} eq 'account' )
  {
    ...
    $text .= '(display name: ' . $self->{'r_line'}->{'given_displayname'} . ') ' unless $self->{'r_line'}->{'given_displayname'} eq '';
    $text .= ' last logon to Skype ';
    $text .= '[timezone policy: ' . $self->{'r_line'}->{'timezone_policy'} . '] ' unless $self->{'r_line'}->{'timezone_policy'} eq '';
    $text .= '<country: ' . $self->{'r_line'}->{'country'} . '> ' unless $self->{'r_line'}->{'country'} eq '';

    $short = $self->{'r_line'}->{'fullname'} . ' (' . $self->{'r_line'}->{'skypename'}  . ') last logged on';
  ...
  elsif ( $self->{'r_type'} eq 'chat' )
  {
    ...
    $self->{'friendlynames'}->{$self->{'r_line'}->{'author'}} = $self->{'r_line'}->{'from_dispname'} unless exists $self->{'friendlyname'}->{$self->{'r_line'}->{'author'}};
    ...
    if( $self->{'r_line'}->{'author'} eq $self->{'account'} )
    {
      ...
      $text .= 'MSG written to';
      $text .= ' ' . $self->{'friendlynames'}->{$self->{'r_line'}->{'dialog_partner'}} if exists $self->{'friendlynames'}->{$self->{'r_line'}->{'dialog_partner'}};
      $text .= ' (' . $self->{'r_line'}->{'dialog_partner'} . ')';
    }
    else
    {
      # then someone is talking to the user
      $text .= 'MSG from ' . $self->{'r_line'}->{'from_dispname'} . ' (' . $self->{'r_line'}->{'author'} . ')';
    }
    #  Chats.friendlyname
    #  Chats.dialog_partner  
    #  Chats.timestamp
  ...
  if( $line eq $magic )
  {
    ...
    if ( -f ${$self->{'name'}} . "-journal" )
    {
      eval
      {
        ...
        copy( ${$self->{'name'}} . "-journal", $temp . "-journal" ) || ($return{'success'} = 0);
      ...
      {
        $return{'success'} = 0;
        $return{'msg'} = 'Database is locked and unable to copy to a temporary location (' . $temp . ')';
      }
    }  
    ...
    eval
    {
      ...
      # get a list of all available talbes
                        $vsth = $self->{'vdb'}->prepare( "SELECT skypeout_precision,chat_policy,nr_of_other_instances,skypename,fullname FROM Accounts" ) or ($temp = 0 );
