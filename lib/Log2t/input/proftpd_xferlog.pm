...
# /^([^\s]+) ([^\s]+) (\d+) (\d\d):(\d\d):(\d\d) (\d\d\d\d) (\d) ([\d\.]+) (\d+) ([^\s]+) ([ab]) ([CUT_]) ([oid]) ([agr]) ([^\s]+) ([^\s]+) ([01]) ([^\s]+) ([ci])/
    ...
    if ($line =~
        /^([^\s]+) ([^\s]+) (\d+) (\d\d):(\d\d):(\d\d) (\d\d\d\d) (\d+) ([\d\.]+) (\d+) ([^\s]+) ([ab]) ([CUT_]) ([oid]) ([agr]) ([^\s]+) ([^\s]+) ([01]) ([^\s]+) ([ci])\s*/
       )
    {
        $li{'day-word'}  = $1;
        $li{'month'}     = $2;
        ...
        print STDERR
          "[PROFTPD XFERLOG] Date after calculation (epoch) $date_e - month: $li{'month'}\n"
          if $self->{'debug'};

        $text =
          "$li{'in?'} $li{'complete?'} transfer of $li{'name'} of $li{'size'} bytes from $li{'r-ip'} using $li{'auth?'} $li{'anon?'} $li{'user'} in $li{'ascii?'} $li{'action'} mode";
        ...
        %t_line = (
               ...
               'extra'      => {
                            ...
                            'authenticated' => $li{'auth?'},
                            'userid'        => $li{'uid'},
                            'complete'      => $li{'complete?'}
                          }
                  );
        return \%t_line;
    }
