...
sub _read_shell_utf($$$)
{
	...
	while( $s )
	{
		...
		printf STDERR "[ShellBag] 0x%x - UTF 0x%x [%s]\n", $$ofs, unpack( "U", $c), encode('utf-8',$c) if $self->{'debug'};
	...
	foreach( @ar )
	{
		if( $_->get_name() eq 'NodeSlot' )
		{
			...
			if( $key_type eq 'ShellNoRoam' )
			{
				...
				print STDERR "[ShellBags] [$num] associated to FOLDER ", $self->{'f_struct'}->{$num}->{'ascii'}, " - ", $self->{'f_struct'}->{$num}->{'utf'}, "\n" if $self->{'debug'};  
				...
				$new_key = $rk->get_subkey( "Software\\Microsoft\\Windows\\$key_type\\Bags\\$num\\Shell") ;#if $self->{'f_struct'}->{$num}->{'os'} eq 'XP';
...
sub _parse_shell_ts()
{
	...
	if( $self->{'vals'}->{$self->{'val_num'}}->{'type'} eq 'ShellNoRoam' )
	{
		...
		$text =  $self->{'f_struct'}->{$self->{'val_num'}}->{'utf'} . ' (' . $self->{'f_struct'}->{$self->{'val_num'}}->{'ascii'} . ')';
		$type = 'ShellNoRoam';
	}
	...
	else
	{
		...
		$text = $ref->{'folder_a'} . '/' . $ref-{'ascii'} .  ' (' . $ref->{'folder_u'} . '/' . $ref->{'utf'} .  ') - ' . $ref->{'type'};
	}
...
sub get_time()
{
	...
	$self->{'key_parse'} = {
		'Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\UserAssist\\{75048700-EF1F-11D0-9888-006097DEACF9}\\Count' => \&_parse_ua,	# UserAssist (XP)
		'Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\UserAssist\\{CEBFF5CD-ACE2-4F4F-9178-9926F41749EA}\\Count' => \&_parse_ua,	# UserAssist (Win 7-1)
		'Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\UserAssist\\{F4E57C4B-2036-45F0-A9AB-443BCFE33D9F}\\Count' => \&_parse_ua,	# UserAssist (Win 7-2)
		'Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\FileExts' => \&_parse_fe,		# FileExts
		'Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\ComDlg32\\LastVisitedMRU' => \&_parse_lvm,		# LastVisitedMRU
		'Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\ComDlg32\\OpenSaveMRU' => \&_parse_osm,		# OpenSaveMRU
		'Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\MapNetworkDriveMRU' => \&_parse_mndm,		# Map Network Drive MRU
		'Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\MountPoints2' => \&_parse_mp2,		# MountPoints2
		'Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\RecentDocs' => \&_parse_rd,		# RecentDocs
