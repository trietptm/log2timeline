...
sub get_time()
{
	...
	%date = (
		...
		hour	=>	$date_t[0],
		minute	=>	$date_t[1],
		time_zone	=> $self->{'zone'} # XP Firewall uses UTC as the timestamp, unless otherwise defined in header
	);
	...
	{
		print STDERR "[WIN FIREWALL] Missing seconds, accuracy is therefore up to the minute (" . $li{'time'} . ")\n" if $self->{'debug'};
	}
	...
	if( $li{'protocol'} eq 'ICMP' )
	{
		...
		$text .= ' ' . $res->{type} . ' - ' . $res->{code} . ' SRC: ' . $li{'src-ip'} . ' > ' . $li{'dst-ip'};

	}
	elsif( $li{'protocol'} eq 'TCP' )
	{
		# TCP
		$text .= ' ' . $li{'protocol'} . ' flags [' . $li{'tcpflags'} . '] window size: ' . $li{'tcpwin'};
		$text .= ' ' . $li{'src-ip'} . ':' . $li{'src-port'} . ' > ' . $li{'dst-ip'} . ':' . $li{'dst-port'} . ' TCP seq [' . $li{'tcpsyn'} . ']';
	}
	else
	{
		# UDP
		$text .= ' ' . $li{'protocol'} . ' ' . $li{'src-ip'} . ':' . $li{'src-port'} . ' > ' . $li{'dst-ip'} . ':' . $li{'dst-port'};
	}
	...
	if( $li{'info'} ne '-' )
	{
        ...
        %t_line = (
                ...
                'version' => 2,
                'extra' => { 'user' => $li{'src-ip'}, 'src-ip' => $li{'src-ip'}, 'host' => $li{'src-ip'}, 'size' => $li{'size'}, 'dst-ip' => $li{'dst-ip'} }
        );
...
sub verify
{
	...
	{
		$return{'msg'} = "There should be " . $self->{'count'} . " words per line, instead there are $#words\n";
		$return{'success'} = 0;
	}
