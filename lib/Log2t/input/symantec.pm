...
sub ReturnAction
{
  ...
  switch($catlevel)
  {
    ...
    case(17){return "Prevent a risk from being logged or a user interface from being displayed";}
    case(18){return "Performing a request to restart the computer";}
    case(19){return "Shows as Cleaned by Deletion in the Risk History in the UI and the Logs in the SSC";}
    case(20){return "Auto-Protect prevented a file from being created; reported \"Access denied\"";}
    default{return $catlevel;}
  }
...
sub get_time()
{
  ...
  $text = 'Category: '.ReturnCategory($fields[2]).(($fields[13])?(' | Description: '.$fields[13]):('')).(($fields[6])?(' | Detected: '.$fields[6]):('')).(($fields[7])?(' | Location: '.$fields[7]):('')).(($fields[10])?(' | Action taken: '.ReturnAction($fields[10])):(''));
