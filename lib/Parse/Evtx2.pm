...
sub get_first_chunk {
	...
	assert(defined $self->{'FH'});
	$self->{'Chunk'} = Parse::Evtx2::Chunk->new(
		'FH' => $self->{'FH'}, 
		'Start' => $self->{'Length'},
	);
	return $self->{'Chunk'};
...
sub get_next_chunk {
	...
	my $chunk = Parse::Evtx2::Chunk->new(
		'FH' => $self->{'FH'}, 
		'Start' => $newstart,
    );
	return $chunk;
...
This method returns a pointer to the current B<Parse::Evtx2::Chunk> object.
...
B<Parse::Evtx2> object on success and C<undef> on failure. Note, that 
...
B<Parse::Evtx2::Event> object on success and C<undef> on failure. Note that 
...
create an instance of B<Parse::Evtx2::Chunk> instead.
...
evtxdump.pl, evtxtemplates.pl, L<Parse::Evtx2::Chunk>, L<Parse::Evtx2::Event>
